...
#include "extensions/filters/listener/dns/dns_server_impl.h"
...
void DnsFilter::onData(Network::UdpRecvData& data) {
  ...
  doDecode(*data.buffer_, data.peer_address_, data.local_address_);
...
void DnsFilter::doDecode(Buffer::Instance& buffer,
                         ...
                         Network::Address::InstanceConstSharedPtr const& local_addr) {
  if (!decoder_) {
    decoder_ = createDecoder();
  }

  try {
    Formats::RequestMessageConstSharedPtr dns_request = decoder_->decode(buffer, from, local_addr);
...
void DnsFilter::onResolveComplete(const Formats::ResponseMessageSharedPtr& dns_message,
                                  Buffer::Instance& serialized_response) {
  Network::UdpSendData send_data{dns_message->localAddr()->ip(), *dns_message->from(), serialized_response};
