...
#include "test/extensions/filters/listener/dns/mocks.h"
...
public:
  ...
  void setup(const std::string qName) {
    ...
    Network::Address::InstanceConstSharedPtr local_addr =
        std::make_shared<Network::Address::Ipv4Instance>("2.2.2.0", 0);
    dns_request_ = std::make_shared<NiceMock<Formats::MockMessage>>(from, local_addr);
    dns_response_ = std::make_shared<NiceMock<Formats::MockMessage>>(from, local_addr);
    ...
    EXPECT_CALL(*dns_resolver_, resolve(_, _, _))
        .WillOnce(Invoke([&](const std::string&, Network::DnsLookupFamily,
          ...
          for (const auto& result : result_list) {
            ret.emplace_back(Network::DnsResponse(result, std::chrono::seconds(0)));
          }
