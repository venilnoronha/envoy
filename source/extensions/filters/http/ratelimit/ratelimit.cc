...
void Filter::complete(RateLimit::LimitStatus status, Http::HeaderMapPtr&& headers) {
  ...
  if (status == RateLimit::LimitStatus::OverLimit &&
    ...
    callbacks_->requestInfo().setResponseFlag(StreamInfo::ResponseFlag::RateLimited);
    ...
    if (config_->failureModeAllow()) {
      ...
      callbacks_->requestInfo().setResponseFlag(StreamInfo::ResponseFlag::RateLimitServiceError);
    }
