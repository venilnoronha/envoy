...
TEST_F(DnsIntegrationTestIpv4ClusterEndpoint, externalDomain) {
  resolver_->resolve(
      ...
      [](const std::list<Network::DnsResponse>&& results) -> void {
        ...
        EXPECT_TRUE(results.front().address_->ip()->ipv4() != nullptr);
      });
}
...
TEST_F(DnsIntegrationTestIpv4ClusterEndpoint, knownDomain) {
  resolver_->resolve(
      ...
      [](const std::list<Network::DnsResponse>&& results) -> void {
        ...
        EXPECT_EQ(results.front().address_->asString(), "127.0.0.100:200");
      });
...
TEST_F(DnsIntegrationTestIpv6ClusterEndpoint, externalDomain) {
  resolver_->resolve(
      ...
      [](const std::list<Network::DnsResponse>&& results) -> void {
        EXPECT_FALSE(results.empty());
        EXPECT_TRUE(results.front().address_->ip()->ipv6() != nullptr);
      });
}
...
TEST_F(DnsIntegrationTestIpv6ClusterEndpoint, knownDomain) {
  resolver_->resolve(
      ...
      [](const std::list<Network::DnsResponse>&& results) -> void {
        EXPECT_EQ(results.size(), 1);
        EXPECT_EQ(results.front().address_->asString(), "::1:200");
      });
