...
Http::FilterHeadersStatus FaultFilter::decodeHeaders(Http::HeaderMap& headers, bool) {
  ...
  if (duration_ms) {
    ...
    callbacks_->requestInfo().setResponseFlag(StreamInfo::ResponseFlag::DelayInjected);
...
void FaultFilter::abortWithHTTPStatus() {
  callbacks_->requestInfo().setResponseFlag(StreamInfo::ResponseFlag::FaultInjected);
